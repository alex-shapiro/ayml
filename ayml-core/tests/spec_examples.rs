...
comments: \"\"\"
  ...
  \"\"\"";
    ...
    assert_eq!(map[&MapKey::String("invoice".into())].value, Value::Int(34843));
    assert_eq!(
        map[&MapKey::String("date".into())].value,
        Value::String("2001-01-23".into())
    );
    assert!(map[&MapKey::String("ship-to".into())].value.is_null());
    assert_eq!(map[&MapKey::String("tax".into())].value, Value::Float(251.42));
    assert_eq!(map[&MapKey::String("total".into())].value, Value::Float(4443.42));

    // bill-to
    let bill_to = map[&MapKey::String("bill-to".into())].value.as_mapping().unwrap();
    assert_eq!(
    ...
    // address
    let address = bill_to[&MapKey::String("address".into())].value.as_mapping().unwrap();
    let lines = address[&MapKey::String("lines".into())].value.as_str().unwrap();
    ...
    // products
    let products = map[&MapKey::String("product".into())].value.as_sequence().unwrap();
    assert_eq!(products.len(), 2);
    let p0 = products[0].value.as_mapping().unwrap();
    assert_eq!(p0[&MapKey::String("sku".into())].value, Value::String("BL394D".into()));
    assert_eq!(p0[&MapKey::String("quantity".into())].value, Value::Int(4));
    assert_eq!(p0[&MapKey::String("price".into())].value, Value::Float(450.0));

    // comments
    let comments = map[&MapKey::String("comments".into())].value.as_str().unwrap();
...
- file: MoreClass.py
  ...
  code: \"\"\"
    ...
    let stack = map[&MapKey::String("Stack".into())].value.as_sequence().unwrap();
    assert_eq!(stack.len(), 2);

    let s0 = stack[0].value.as_mapping().unwrap();
    assert_eq!(s0[&MapKey::String("file".into())].value, Value::String("TopClass.py".into()));
...
rbi: 147   # Runs Batted In";
    ...
    assert_eq!(map[&MapKey::String("avg".into())].value, Value::Float(0.278));
...
Sammy Sosa: {
  ...
  avg: 0.288,
    ...
    let mcgwire = map[&MapKey::String("Mark McGwire".into())].value.as_mapping().unwrap();
    assert_eq!(mcgwire[&MapKey::String("hr".into())].value, Value::Int(65));

    let sosa = map[&MapKey::String("Sammy Sosa".into())].value.as_mapping().unwrap();
...
- Ken Griffey";
    ...
    let hr = map[&MapKey::String("hr".into())].value.as_sequence().unwrap();
    assert_eq!(hr.len(), 2);
    assert_eq!(hr[0].value, Value::String("Mark McGwire".into()));

    let rbi = map[&MapKey::String("rbi".into())].value.as_sequence().unwrap();
    assert_eq!(rbi.len(), 2);
...
network:
  rules:
    ...
    let map = node.value.as_mapping().unwrap();
    let network = map[&MapKey::String("network".into())].value.as_mapping().unwrap();
    let rules = network[&MapKey::String("rules".into())].value.as_sequence().unwrap();
    ...
    let ports = rule0[&MapKey::String("ports".into())].value.as_sequence().unwrap();
